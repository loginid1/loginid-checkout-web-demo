...
import "./App.css";
import { FidoVaultSDK, PostTxnsResult, WalletTransaction } from "./lib/LoginidVaultSDK";
...
function App() {
	...
	async function postTransaction(signTxns: string[]): Promise<boolean> {
		...
		const confirmedTxn = await algosdk.waitForConfirmation(algodv2, tx.txId, 4);
	...
	async function handleGroupClick() {
		try {
			...
			*/
			...
			const txnb64_1 = Buffer.from(txgroup[0].toByte()).toString("base64");
			let wTxn1: WalletTransaction = {
				txn: txnb64_1,
			};

			const txnb64_2 = Buffer.from(txgroup[1].toByte()).toString("base64");
	...
	async function handleGroupClick3() {
		try {
			...
			const txnb64_3 = Buffer.from(txgroup[2].toByte()).toString("base64");
	...
	return (
		<ThemeProvider theme={theme}>
			<Container component="main" maxWidth="xs">
				...
				<Card>
					<CardHeader title="FIDO Vault Connections"></CardHeader>
					<CardContent>
						{enableAccount ? (
							<>
								<Typography>
				...
				<Card sx={{ mt: 2 }}>
					<CardHeader title="Algorand Transaction Tests"></CardHeader>
					<CardContent>
						<Stack>
						<Button size="small" variant="outlined" onClick={handleTransactionClick} >
							SINGLE PAYMENT	
						</Button>
						<Button size="small" variant="outlined" onClick={handleAssetOptinClick}>
							ASSET OPT-IN	
						</Button>
						<Button size="small" variant="outlined" onClick={handleGroupClick}>
							GROUP TRANSACTION #2
						</Button>
						<Button size="small" variant="outlined" onClick={handleGroupClick3}>
							GROUP TRANSACTION #3
						</Button>
						<Button size="small" variant="outlined" onClick={generateSuggestedParams}>
							SUGGESTED PARAMS	
						</Button>
						</Stack>
				...
				{enableAccount && (
					<Card sx={{ mt: 2 }}>
						<CardHeader title="Algorand Deposit Test"></CardHeader>
						<CardContent>
