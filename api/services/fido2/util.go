package fido2

import (
	"encoding/base64"
	"encoding/binary"
	"encoding/json"
	"errors"

	"github.com/fxamacker/cbor/v2"
	logger "gitlab.com/loginid/software/libraries/goutil.git/logger"
	"gitlab.com/loginid/software/services/loginid-vault/services"
)

/*
CBOR decode
attestData = {fmt: string , attStmt , authData}

grab authData

grab COSE data

extract alg from COSE expecting(1:1,3:-7,-1:1,-2:X,-3:Y)
extrace key data X & Y from COSE
COSE TYPE (1) - create sign

COSE ALG (3)
ES256 | -7    | SHA-256 | ECDSA w/ SHA-256 |
              | ES384 | -35   | SHA-384 | ECDSA w/ SHA-384 |
              | ES512 | -36   | SHA-512 | ECDSA w/ SHA-512 |

			  -1 Curve Type P256
			  -2 X
			  -3 Y
*/

type AttestObject struct {
	Fmt      string          `cbor:"fmt"`
	AttStmt  AttestStatement `cbor:"attStmt",omitempty`
	AuthData []byte          `cbor:"authData`
}

type AttestStatement struct {
	Alg int8   `cbor:"alg,omitempty"`
	Sig []byte `cbor:"sig,omitempty"`
}

func ExtractPublicKey(attestation_data string) (string, string, *services.ServiceError) {

	var attestObject AttestObject
	data, err := base64.RawURLEncoding.DecodeString(attestation_data)
	if err != nil {
		logger.Global.Error(err.Error())
		return "", "", services.CreateError("invalid FIDO public key")
	}
	cbor.Unmarshal(data, &attestObject)

	coseBytes, err := parseCoseData(attestObject.AuthData)
	if err != nil {
		logger.Global.Error(err.Error())
		return "", "", services.CreateError("failed to parse public key")
	}

	var cose CoseData
	cbor.Unmarshal(coseBytes, &cose)
	if err != nil {
		logger.Global.Error(err.Error())
		return "", "", services.CreateError("invalid FIDO public key")
	}

	/*
		if cose.Kty != CoseEC2 && cose.Alg != Cose_ES256 {
			logger.Global.Info(fmt.Sprintf("PK alg: %d", attestObject.AttStmt.Alg))
			return "", "", services.CreateError("unsupported public key")
		}
	*/

	//logger.Global.Info(fmt.Sprintf("%#v ", coseBytes))
	//logger.Global.Info(fmt.Sprintf("%#v %d", cose, attestObject.AttStmt.Alg))
	//public_key := "HNVQH74DGRLCQEQXNSLVYKTX5F5HXMS2M6IZJX4VFQRUUSNYEQ7MB5AV6E"
	//logger.Global.Info(fmt.Sprintf("X: %s", base64.StdEncoding.EncodeToString(cose.XOrE)))
	//logger.Global.Info(fmt.Sprintf("Y: %s", base64.StdEncoding.EncodeToString(cose.Y)))
	if cose.Kty == CoseEC2 {

		jwk := convertToEccJWK(cose)
		public_key, err := json.Marshal(jwk)
		if err != nil {
			logger.Global.Error(err.Error())
			return "", "", services.CreateError("invalid FIDO public key")
		}
		return string(public_key), jwk.Crv, nil
	} else if cose.Kty == CoseRSA {
		jwk, err := convertToRsaJWK(cose)
		if err != nil {
			logger.Global.Error(err.Error())
			return "", "", services.CreateError("invalid RSA public key")
		}
		public_key, err := json.Marshal(jwk)
		if err != nil {
			logger.Global.Error(err.Error())
			return "", "", services.CreateError("invalid FIDO public key")
		}
		return string(public_key), jwk.Alg, nil

	}
	return "", "", services.CreateError("unsupported public key")
}

type CoseConsts int

const (
	CoseOKP           CoseConsts = 1
	CoseEC2           CoseConsts = 2
	CoseRSA           CoseConsts = 3
	CoseECDHESHKDF256 CoseConsts = -25
	CoseES256         CoseConsts = -7
)

type CoseAlg int

const (
	Cose_ES256 CoseAlg = -7
	Cose_ES384 CoseAlg = -35
	Cose_ES512 CoseAlg = -36
	Cose_RS256 CoseAlg = -257
	Cose_RS384 CoseAlg = -258
	Cose_RS512 CoseAlg = -259
	Cose_PS256 CoseAlg = -37
	Cose_PS384 CoseAlg = -38
	Cose_PS512 CoseAlg = -39

	// ----- FIPS note -----
	// As per https://nvlpubs.nist.gov/nistpubs/SpecialPublications/NIST.SP.800-131Ar1.pdf
	// It is not allowed to GENERATE SHA1 signature, but is allowed to VERIFY SHA1 SIGNATURES
	// SO DO NOT REMOVE
	Cose_RS1 CoseAlg = -65535

	// ----- FIPS note -----
	// As per https://csrc.nist.gov/CSRC/media/Projects/cryptographic-module-validation-program/documents/fips%20140-3/FIPS%20140-3%20IG.pdf
	// It is allowed to support EdDSA with ED25519 or ED448
	// ----- MAY HAVE COMPATIBILITY ISSUE WITH LATEST BoringCrypto FIPS -----
	// SO DO NOT REMOVE
	Cose_ED CoseAlg = -8
)

// 32 bytes rpID
// 1 byte flag
// 4 bytes counter
// 16 bytes aaguid
// 2 bytes credlength
// X bytes credID (aka keyHandle)
// X bytes COSE data

type CoseData struct {
	Kty    CoseConsts  `cbor:"1,keyasint"`  // 1:2
	Alg    CoseAlg     `cbor:"3,keyasint"`  // 3:-7 for es256 3:3 for RSA
	CrvOrN interface{} `cbor:"-1,keyasint"` // -1:1 for es256
	XOrE   []byte      `cbor:"-2,keyasint"`
	Y      []byte      `cbor:"-3,keyasint"`
}

const CRED_LENGTH_INDEX = 53

func parseCoseData(authData []byte) ([]byte, error) {

	credLen := authData[CRED_LENGTH_INDEX : CRED_LENGTH_INDEX+2]
	coseLength := binary.BigEndian.Uint16(credLen)
	coseIndex := CRED_LENGTH_INDEX + 2 + coseLength
	coseData := authData[coseIndex:]
	return coseData, nil
}

func convertToRsaJWK(cose CoseData) (*services.RsaJWK, error) {
	if cose.Kty != CoseRSA {
		return nil, errors.New("RSA N only available for RSA key format.")
	}

	resultAlg := "RS256"
	switch cose.Alg {
	case Cose_PS256:
		resultAlg = "PS256"
	case Cose_PS384:
		resultAlg = "PS384"
	case Cose_PS512:
		resultAlg = "PS512"
	case Cose_RS256:
		resultAlg = "RS256"
	case Cose_RS384:
		resultAlg = "RS384"
	case Cose_RS512:
		resultAlg = "RS512"
	case Cose_RS1:
		resultAlg = "RS1"
	}

	key := services.RsaJWK{
		Kty: "RSA",
		Alg: resultAlg,
		N:   base64.RawURLEncoding.EncodeToString(cose.CrvOrN.([]byte)),
		E:   base64.RawURLEncoding.EncodeToString(cose.XOrE),
	}
	return &key, nil
}

func convertToEccJWK(cose CoseData) services.EccJWK {

	crv := "P-256"
	if cose.CrvOrN == 2 {
		crv = "P-384"
	} else if cose.CrvOrN == 3 {
		crv = "P-521"
	}

	key := services.EccJWK{
		Kty: "EC",
		Crv: crv,
		X:   base64.RawURLEncoding.EncodeToString(cose.XOrE),
		Y:   base64.RawURLEncoding.EncodeToString(cose.Y),
	}
	return key
}
