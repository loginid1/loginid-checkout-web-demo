...
export class TrustID {
     ...
     * @returns 
     */
    static async create(username: string, challenge: string, challengeType: string): Promise<TrustIDResult> {
     ...
     * @returns 
     */
    static async sign(username: string, challenge: string, challengeType: string): Promise<TrustIDResult> {
        ...
        if (dbResult) {
            ...
            const token = await TrustIDHelper.buildTrustJWT(dbResult.key, header, claims);
            return {id: id, token: token};
        } else {
            throw new Error("error storing trust db")
        }
...
class TrustIDHelper {
    ...
    static async storeKeystore(id: string, username: string, key: CryptoKeyPair): Promise<boolean> {
     ...
     */
    static async sign(key: CryptoKeyPair, payload: ArrayBuffer): Promise<ArrayBuffer> {
        return await window.crypto.subtle.sign(this.ES256_ALGORITHM, key.privateKey, payload)
    }

    static async verify(key: CryptoKeyPair, signature: ArrayBuffer, challenge: ArrayBuffer): Promise<boolean> {
        return await window.crypto.subtle.verify(this.ES256_ALGORITHM, key.publicKey, signature, challenge)
    }

    static async buildTrustJWT(keypair: CryptoKeyPair, header: TrustHeader, claims: TrustClaims): Promise<string> {
