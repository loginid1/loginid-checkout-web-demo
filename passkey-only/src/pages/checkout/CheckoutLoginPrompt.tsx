...
export default function CheckoutLoginPrompt(props: CheckoutLoginPromptProps) {
  ...
  async function handleAutoFill() {
    try {
      const result = await LoginidService.client.authenticateWithPasskeyAutofill({ abortController: abortController });
      //return router("/manage");
      const token = ParseUtil.parseToken(result.idToken);
      //console.log(token);
      return props.onComplete(token["cognito:username"], token,  "next");
  ...
  const handlerSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (password !== "") {
        const result = await LoginidService.client.authenticateWithPassword(email, password);
        // complete signin and go to create passkey suggestion
        return props.onComplete(email, result.getIdToken().getJwtToken(), "passkey");

      } else {
        const result = await LoginidService.client.authenticateWithPasskey(email, { abortController});
        if(result.isAuthenticated) {
          return props.onComplete(email, result.idToken, "checkout");
        } else {
