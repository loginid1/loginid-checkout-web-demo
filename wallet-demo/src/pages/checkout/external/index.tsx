...
export function CheckoutExternalPage() {
  ...
  async function confirmExternal() {
    ...
    try {
      const bresult = await WalletMockService.bankingResult(
        bResult.id,
        bResult.username,
      );
      setUsername(bResult.username);
      // confirm external with loginID
      const result = await LIDService.client.performAction("external", {
        payload: bresult.token,
      });
      ...
      if (result.nextAction == "passkey:reg") {
        ...
        const baseCallback =
          sorder.callback || `http://localhost:3001/callback`;
        document.location.href =
          baseCallback +
          "?data=" +
          ParseUtil.consertJSONToB64({ error: "unconfirmed purchase" });
      }
    } catch (e) {
