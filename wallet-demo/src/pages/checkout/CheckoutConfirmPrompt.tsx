...
export function CheckoutConfirmPrompt(props: CheckoutConfirmPromptProps) {
    ...
    async function confirmPayTranstion() {
        // clear prior result
        if (props.token != "") {
            return props.onComplete(props.username, props.token, props.hasPasskey?"passkey":"none");
        }
        clear()
        try {
            console.log(props.username);
            const result = await LIDService.client.performAction("passkey:tx", {txPayload:JSON.stringify(payData)})
            //const result = await LIDService.client.performAction("passkey:tx")
            if(result.payloadSignature){
                return props.onComplete(props.username, result.payloadSignature, "passkey");
            }
        } catch (e: any) {
