...
export default class CheckoutSDK {
	...
	prepareIframe(): Window | null {
		...
		//main.style.right = "0";
		main.style.left = "" + (window.innerWidth/2 - 320/2 ) + "px" ;
		main.style.top = "" +  (window.innerHeight - 452) + "px"  ;
		...
		main.animate([{ transform: "translateY(100%)" }, { transform: "translateY(0)" }], {duration:750});
		this.mMain = main;
	...
	async preID(): Promise<IDResult> {
		...
		/*
		let isLoad = await mMessage.pingForResponse(target, 20000);
		if (!isLoad) {
			return Promise.reject({ message: "communication timeout" });
		}
		let response = await mMessage.sendMessage(
		...
		*/
		let response = await mMessage.pingForID(target, 1000);
